  ...
  calculatePath(req, res) {
    try {
      ...
      if (parameterCount === 8) {
        ...
        const { mapId, startX, startY, startZ, angle, hoverHeight, objectSize, collisionHeight } = req.query;
        ...
        const path = Navigation.moveForward(mapIdParsed, parseFloat(startX), parseFloat(startY), parseFloat(startZ), parseFloat(angle), parseFloat(hoverHeight), parseFloat(objectSize), parseFloat(collisionHeight));
