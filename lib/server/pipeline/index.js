...
class Pipeline {
  ...
  calculatePath(req, res) {
    try {
      ...
      const { startX, startY, startZ, endX, endY, endZ } = req.query;
      //console.log("startX: " + startX);
      //console.log("endX: " + endX);
      const path = Navigation.calculatePath(parseFloat(startX), parseFloat(startY), parseFloat(startZ), parseFloat(endX), parseFloat(endY), parseFloat(endZ));
  ...
  resource(req, _res, next, path) {
    if (path === 'calculatePath') {
        // Directly call the calculatePath method
        // Make sure to bind 'this' to ensure calculatePath has access to the Pipeline instance's properties and methods
