...
class Pipeline {
  ...
  constructor() {
    ...
    this.router.get('/getClosestNode', this.getClosestNode.bind(this));
  }

  getClosestNode(req, res) {
    const parameterCount = Object.keys(req.query).length;
    if (parameterCount === 1) {
      const node_id = parseInt(req.query.node_id);
      const closestNode = nodeManager.getRandomLinkedNode(node_id);

      // Check if a node was found
      if (closestNode) {
        //console.log('New linked node:', closestNode);
        res.json(closestNode);
      } else {
        res.status(404).send("No nodes available or nodes not loaded yet.");
      }
      return;
    }
    ...
    if (isNaN(x) || isNaN(y) || isNaN(z)) {
      return res.status(400).send("Invalid coordinates supplied");
    }

    const myPosition = { x, y, z };
    const closestNode = nodeManager.getClosestNode(myPosition);

    // Check if a node was found
    if (closestNode) {
      console.log('Closest Node:', closestNode);
      res.json(closestNode); // Send the closest node as a JSON response
    } else {
      res.status(404).send("No nodes available or nodes not loaded yet.");
    }
  }
  ...
  calculatePath(req, res) {
    try {
      ...
      if (parameterCount === 9) {
        ...
        const { mapId, startX, startY, startZ, angle, hoverHeight, objectSize, collisionHeight, dist } = req.query;
        ...
        const path = Navigation.moveForward(mapIdParsed, parseFloat(startX), parseFloat(startY), parseFloat(startZ), parseFloat(angle), parseFloat(hoverHeight), parseFloat(objectSize), parseFloat(collisionHeight), parseFloat(dist));
        ...
        const { startX, startY, startZ, endX, endY, endZ, mapId, straightPath, hoverHeight, objectSize, collisionHeight } = req.query;
        const mapIdParsed = parseInt(mapId, 0);
        ...
        const path = Navigation.calculatePath(parseFloat(startX), parseFloat(startY), parseFloat(startZ), parseFloat(endX), parseFloat(endY), parseFloat(endZ), mapIdParsed, straightPathBool, parseFloat(hoverHeight), parseFloat(objectSize), parseFloat(collisionHeight));
  ...
  resource(req, _res, next, path) {
    ...
    } else if (path === 'getClosestNode') {
        return this.getClosestNode(req, _res);
    }
